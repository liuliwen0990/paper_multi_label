...
class MLCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=32, kernel_size=(3, 3), stride=1, padding=(1, 1)),
            nn.BatchNorm2d(32),
            nn.ReLU(),
        )
        # 32*2*128
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(1, 3), stride=1, padding=0),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        # 64*4*128
        ...
        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(2, 3), stride=1, padding=(0, 0)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
    ...
    for kk in range(0, len(snr_val)):
        for mm in range(0, len(train_val)):
            ...
            for l in range(num_experiment):
                ...
                n_epoch = 150
                patience = 20  # 当验证集损失在连续15次训练周期中都没有得到降低时，停止模型训练，以防止模型过拟合
            ...
            filename =  '0704_results_MLCNN2_snr_{}_trainnum_{}.xls'.format(SNR,train_num)
